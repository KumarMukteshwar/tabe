...
const STICKY_COUNT   = 3;   // first 3 columns (by current order) are frozen
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: "#18181b", marginBottom: 6 }}>
          Move Column
        </div>
        <div style={{ fontSize: 13, color: "#71717a", lineHeight: 1.6, marginBottom: 22 }}>
          ...
          <br />
          This will reorder the visible columns.
        </div>
...
export function App() {
  ...
  // ── Drag state ────────────────────────────────────────────────────────────
  const dragCol      = useRef(null);   // index being dragged
  const [dragOver, setDragOver]   = useState(null);   // index being hovered over
  const [isDragging, setIsDragging] = useState(false);

  // ── Confirm modal state ───────────────────────────────────────────────────
  const [pendingMove, setPendingMove] = useState(null);  // { fromIdx, toIdx }
  ...
  const onDragStart = (e, ci) => {
    ...
    // ghost image: transparent 1×1 pixel
  ...
  const onDragOver = (e, ci) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = "move";
    if (ci !== dragCol.current) setDragOver(ci);
  };
  ...
  // ── Apply column move after confirmation ──────────────────────────────────
  const applyMove = () => {
  ...
  const totalWidth = CHECKBOX_WIDTH + colWidths.reduce((a, b) => a + b, 0);

  // ── Colors (all fully opaque) ─────────────────────────────────────────────
                    ...
                    return (
                      ...
                      >
                        <div style={{ position: "relative", display: "flex", alignItems: "center", gap: 5, height: "100%" }}>
                          ...
                          <span style={{ overflow: "hidden", textOverflow: "ellipsis", fontFamily: "'IBM Plex Mono', monospace" }}>
                            {col.label}
                          </span>
                          ...
                          {/* {isSticky && (
                            <span style={{
                              marginLeft: "auto", background: "#e0e7ff", color: "#6366f1",
                              borderRadius: 4, fontSize: 9, fontWeight: 700,
                              padding: "1px 5px", letterSpacing: "0.04em", flexShrink: 0,
                            }}>PIN</span>
                          )} */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td onClick={() => toggleRow(user.id)} style={{
                        padding: "0 14px",
                        borderBottom: `1px solid ${BORDER}`, borderRight: `1px solid ${BORDER_STK}`,
                        background: stickyBg,
                          ...
                          <td key={col.id} style={{
                            ...
                            {col.id === "status" ? (
                              ...
                              <span style={{
                                ...
                                color: col.id === "name" ? TEXT_PRIMARY : col.id === "role" ? TEXT_ROLE : TEXT_SEC,
                              }}>
                                {user[col.id]}
                              </span>
                            )}
